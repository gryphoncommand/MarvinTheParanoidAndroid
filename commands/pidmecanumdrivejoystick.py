...
class PIDMecanumDriveJoystick(Command):
    ...
    def __init__(self):
        ...
        self.pid["lr"] = PIDController(pid.R[0], pid.R[1], pid.R[2], pid.R[3], src, drive.set_lr)
        self.pid["rf"] = PIDController(pid.R[0], pid.R[1], pid.R[2], pid.R[3], src, drive.set_rf)
        self.pid["rr"] = PIDController(pid.R[0], pid.R[1], pid.R[2], pid.R[3], src, drive.set_rr)
        ...
        self.max_range = (-.15, .15)
        self.min_range = (-.85, .85)
    ...
    def execute(self):
        ...
        subsystems.drivetrain.driveCartesian(oi.joystick.getX(), oi.joystick.getY(), oi.joystick.getZ(),0)
